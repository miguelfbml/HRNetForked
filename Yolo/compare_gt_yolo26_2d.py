...
Standalone entrypoint for the YOLO26l model trained by train_yolo26l_pose.py.
It loads the trained weights from Yolo/weights/best.pt by default and writes
outputs to Yolo/comparison_output_yolo26/.
...
def compute_pck_2d(pred_poses, gt_poses, torso_diameters, fixed_threshold=150.0):
    ...
    results[f"PCK@100%_{int(fixed_threshold)}px"] = np.mean(joint_distances < fixed_threshold)
    return results


def compute_auc_2d(pred_poses, gt_poses, max_threshold=150.0, num_thresholds=50):
    if pred_poses.shape != gt_poses.shape:
        print("Warning: Shape mismatch for AUC calculation")
        return 0.0
...
def _recursive_find_sequence(node, sequence_name):
    if isinstance(node, dict):
        if sequence_name in node:
            return node[sequence_name]
        for value in node.values():
            found = _recursive_find_sequence(value, sequence_name)
            if found is not None:
                return found
    return None


def _load_npz_data(npz_path):
    if not npz_path or not os.path.exists(npz_path):
        return None
    try:
        return np.load(npz_path, allow_pickle=True)["data"].item()
    except Exception as exc:
        print(f"Failed to load npz data from {npz_path}: {exc}")
        return None


def _get_ground_truth_candidates():
    return [
        REPO_ROOT / "motion3d" / "data_test_3dhp.npz",
        REPO_ROOT / "motion3d" / "data_3dhp_test.npz",
        SCRIPT_DIR / ".." / "motion3d" / "data_test_3dhp.npz",
    ]


def get_available_sequences():
    for candidate in _get_ground_truth_candidates():
        data = _load_npz_data(str(candidate))
        if data:
            print(f"✓ Found ground truth data at: {candidate.resolve()}")
...
def load_test_3d_data_from_dataset(sequence_name):
    for candidate in _get_ground_truth_candidates():
        ...
        seq_data = data.get(sequence_name)
        if seq_data is None:
            seq_data = _recursive_find_sequence(data, sequence_name)
        ...
        try:
            ...
            poses_3d = seq_data.get("data_3d") if isinstance(seq_data, dict) else None
            print(f"✓ Loading ground truth from: {candidate.resolve()}")
            return poses_2d, poses_3d, sequence_name
        except Exception:
            continue

    print(f"❌ Sequence {sequence_name} not found in ground truth data")
    for candidate in _get_ground_truth_candidates():
        data = _load_npz_data(str(candidate))
        if data:
            print(f"   Available top-level keys in {candidate.name}: {list(data.keys())[:10]}")
...
def load_test_frames(sequence_name, num_frames=None):
    frames, _ = load_test_frames_batch(sequence_name, 0, num_frames)
    return frames
...
def estimate_yolo_poses_batch(model, frames, img_size=640, device="cpu"):
    ...
    yolo_poses = np.array(yolo_poses)
...
def compute_mpjpe_2d(gt_poses_2d, yolo_poses_2d):
    ...
    for frame_idx in range(min_frames):
        ...
        if gt_valid and yolo_valid:
            ...
            frame_mpjpe.append(np.mean(np.linalg.norm(gt_frame - yolo_frame, axis=1)))
        else:
    ...
    return {
        ...
        "auc": float(auc),
        "valid_frames": len(valid_gt_list),
        ...
        for start_idx in range(0, total_gt_frames, batch_size):
            ...
            for frame_idx in range(min_frames):
                ...
                if gt_valid and yolo_valid:
                    ...
                    all_frame_mpjpe.append(np.mean(np.linalg.norm(gt_frame - yolo_frame, axis=1)))
                else:
...
def create_comparison_visualization(gt_poses_2d, yolo_poses_2d, seq_name, metrics, args):
    ...
    if metrics:
        title = (
            f"2D Pose Comparison: GT vs YOLO26 - {seq_name} (Root-Relative, Pixels)\n"
        ...
        title = f"2D Pose Comparison: GT vs YOLO26 - {seq_name} (Root-Relative, Pixels)"
    ...
    def update(frame_idx):
        ...
        if not np.all(gt_frame == 0):
            ...
            ax1.text((x_min + x_max) / 2, (y_min + y_max) / 2, "No GT Data", ha="center", va="center", fontsize=16, color="red")

        frame_mpjpe_val = frame_mpjpe[frame_idx] if not np.isnan(frame_mpjpe[frame_idx]) else 0
        ax2.set_title(f"YOLO26 Estimation\nFrame {frame_idx + 1}/{min_frames} | Frame MPJPE: {frame_mpjpe_val:.1f}px", fontsize=14)
        yolo_frame = yolo_poses[frame_idx]
        if not np.all(yolo_frame == 0):
            ...
            ax2.text((x_min + x_max) / 2, (y_min + y_max) / 2, "No YOLO Detection", ha="center", va="center", fontsize=16, color="red")

        for ax in [ax1, ax2]:
            ax.scatter(0, 0, c="green", s=120, marker="*", alpha=1.0, edgecolors="darkgreen", linewidth=2, label="Root (Hip Center)")
...
def print_summary_results(all_metrics, model_name):
    ...
    print(f"\n{'=' * 80}")
    print("COMPREHENSIVE SUMMARY - YOLO26 vs Ground Truth (2D)")
    ...
    for metrics in valid_metrics:
        print(f"{metrics['sequence']:<10} {metrics['avg_mpjpe']:<12.2f} {metrics['auc']:<10.4f} {metrics['performance']['fps']:<10.2f} {metrics['performance']['mean_inference_time'] * 1000:<12.2f} {metrics['valid_frames']}/{metrics['total_frames']:<12}")

    print(f"{'=' * 80}")
    print(f"FINAL OVERALL MPJPE: {overall_mpjpe:.2f} pixels")
    print(f"FINAL OVERALL FPS: {overall_fps:.2f}")
    print(f"FINAL MEAN INFERENCE TIME: {overall_mean_inference_time * 1000:.2f} ms")
    print(f"{'=' * 80}")


def main():
    parser = argparse.ArgumentParser(description="Compare Ground Truth and YOLO26 2D poses with comprehensive metrics")
    parser.add_argument("--sequence", type=str, default="TS1", help="Sequence to compare (TS1, TS2, TS3, TS4, TS5, TS6)")
    parser.add_argument("--model-path", type=str, default=str(DEFAULT_MODEL_PATH), help="Path to trained YOLO26l model (.pt file)")
    parser.add_argument("--num-frames", type=int, default=None, help="Number of frames to process per sequence")
    parser.add_argument("--all", action="store_true", help="Run evaluation on all available sequences")
    parser.add_argument("--save-video", action="store_true", help="Save comparison as GIF (only for single sequence)")
    parser.add_argument("--output-dir", type=str, default=str(DEFAULT_OUTPUT_DIR), help="Directory to save outputs")
    ...
    print("YOLO26 Ground Truth vs 2D Pose Comparison with Comprehensive Metrics")
    print("=" * 80)
    ...
    print("Metrics: MPJPE, PCK, AUC, FPS, Inference Time")
    ...
    if args.all:
        ...
        if args.num_frames is None:
            print("Processing ALL frames from ALL sequences using memory-efficient batching.")
        ...
        for sequence in available_sequences:
            try:
                ...
                print(f"❌ Error processing sequence {sequence}: {exc}")
        ...
        if args.save_video or not args.all:
            ...
            if gt_poses_2d is not None and frames is not None:
                ...
                try:
                    result = create_comparison_visualization(gt_poses_2d_root_rel, yolo_poses_2d_root_rel, args.sequence, metrics, args)
                    if result[0] is not None:
                        ...
                        if args.save_video:
                            ...
                            output_path = os.path.join(args.output_dir, f"{args.sequence}_gt_vs_yolo26_{model_name_clean}_comprehensive.gif")
                            ani.save(output_path, writer="pillow", fps=3, dpi=100)
                            print(f"✓ Animation saved to: {output_path}")

                            update_func(0)
                            static_path = os.path.join(args.output_dir, f"{args.sequence}_gt_vs_yolo26_{model_name_clean}_comprehensive.png")
